...
public class Vehiculo {
    ...
    public static String toColorVehiculos(String color_nuevo){
        String color_Vehiculos = "";
 
        for(int i = 0; i<vehiculos.length; i++){
            for(int j = 0; j<vehiculos[i].length; j++){
                if(vehiculos[i][j] != null){
                    if(color_nuevo.equalsIgnoreCase(vehiculos[i][j].getColor())){
                        color_Vehiculos= color_Vehiculos + vehiculos[i][j].toString() +   ","+ vehiculos[i][j].imprimir()+"\n";
                    }
                }
            }
        }
    ...
    public static String ordenarVehiculoValor(Vehiculo [][] nuevo){
        ...
        for(contador = 0; contador<temporal.length;contador++){
            if(temporal[contador] != null){
                arreglo_ordenado = arreglo_ordenado + temporal[contador].toString() +  ","+ temporal[contador].imprimir()+ "\n";
            }
        }
    ...
    public static String toStringVehiculos(){
        ...
        for(int i = 0; i<vehiculos.length; i++){
            for(int j = 0; j<vehiculos[i].length; j++){
                if(vehiculos[i][j] != null){
                    informacion_vehiculo = informacion_vehiculo + vehiculos[i][j].toString() +  ","+ vehiculos[i][j].imprimir()+" y fue ingresado "+vehiculos[i][j].hora1+"\n";
                }
            }
        }
